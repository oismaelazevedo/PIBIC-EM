...
for k in range(200):
    ...
    numBaseInverso = Rational(1, numBase)
    ...
    dados = {
        'equacaoExponencial' : [
            {
                'numBase' : numBase,
                'resposta': resposta,
                'numBaseInverso': pretty(numBaseInverso)
            }
        ],
        'respostas': [
            {
        ...
        'atributosquestao': [
            {
                'enunciado': 'Resolva a seguinte equação exponencial: ({})^x = {}'.format(numBaseInverso, numBase ** -resposta),
