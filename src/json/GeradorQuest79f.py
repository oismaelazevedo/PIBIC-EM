...
for k in range(50):
    ...
    questoes = open("questoes{}-79f.json".format(k+1),'w')

    numBase = rnd.randint(2,20)
    numExpoenteNumBase = rnd.randint(1,10)
    numMultiplicadorIncognitaAlternativa = (2 * (numBase ** 4)) - (5 * (numBase ** 2)) - (3 * numBase) - 1

    resposta = round(numExpoenteNumBase/2, 2)

    x = Symbol('x')
    express = (2 * ((numBase ** 2) ** (x+2))) - (5 * ((numBase ** 2) ** (x+1))) - (3 * (numBase ** (2*x + 1))) - ((numBase ** 2) ** x) 
        ...
        for numLetra in range(0,5):
            if questaoCerta == listLetra[numLetra]:
                ...
                if numRandomTemporario == 0:
                    listAlternativas[numLetra] = round(numExpoenteNumBase/3, 2)
                ...
                if numRandomTemporario == 0:
                    listAlternativas[numLetra] = round(rnd.randint(2,20)/2, 2)
                    isCorrect[numLetra] = "Nao"
